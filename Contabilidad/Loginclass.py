...
class log:
    def __init__(self,ven):
        ...
        self.vp.title("INICIO")
        self.vp.resizable(False,False)
       ...
       #label_imagen.place(y=0, x=0)
        ...
        #---------Creamos un frame----------------
        frame = customtkinter.CTkFrame(master=ven, fg_color="DeepSkyBlue4", 
                                    border_color="DeepSkyBlue4",width=327, height=550)
        frame.pack(side=tk.RIGHT, fill=tk.BOTH)

        #--------Dentro del frame----------------
        label = customtkinter.CTkLabel(master=frame, text="BIENVENIDO",
                                    bg_color="DeepSkyBlue4",text_color="black", font=('Arial', 30, 'bold', ))
        ...
        marco = LabelFrame(master=frame, text="INICIA SESIÓN", 
                             bg="DeepSkyBlue4",font=("Comic Sans", 15, "bold"))
        marco.config(padx=30, pady=10, bd=5)
        marco.pack()

        matricula_label = tk.Label(marco, text="No.EMPLEADO:",
                                     bg="DeepSkyBlue4", font=("Arial", 12, "bold"))
        matricula_label.grid(row=1, column=1, padx=5, pady=8)
     #vcmd = (marco.register(validate_matricula), '%P') validatecommand=vcmd

        matricula_entry = tk.Entry(marco, validate="key",width=20, 
                                     font=("Arial", 12), textvariable=verify_m)
        ...
        password_label = tk.Label(marco, text="CONTRASEÑA:", bg="DeepSkyBlue4", 
                               font=("Arial", 12, "bold"))
        password_label.grid(row=2, column=1, padx=5, pady=8)
     #vcmd2 = (marco.register(validate_password), '%P') validatecommand=vcmd2

        password_entry = tk.Entry(marco, show="★", validate="key", width=20, 
                               font=("Arial", 12), textvariable=verify_p)
...
#NumEmp =customtkinter
        ...
        def register(self,vn):
                ...
                self.v2=vn
                vn=CTk()
                self.v2.geometry("400x300")
                self.v2.title("REGISTRO")
                self.v2.config(bg="black")
